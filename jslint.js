...
/*
     FILE ARCHIVED ON 17:10:01 Jun 27, 2007 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 14:59:51 May 25, 2015.
...
quit=function quit(m,l,ch){warningAt("{a} ({b}% scanned).",l,ch,m,Math.floor((l/lines.length)*100));JSLINT.errors.push(null);throw null;};var lex=function(){var character,from,line,s;function nextLine(){line+=1;if(line>=lines.length){return false;}
character=0;s=lines[line];return true;}
function it(type,value){var i,t;if(option.adsafe&&adsafe.hasOwnProperty(value.toLowerCase())){warning("Adsafe restricted word '{a}'.",{line:line,from:character},value);}
if(type==='(punctuator)'||(type==='(identifier)'&&syntax.hasOwnProperty(value))){t=syntax[value];if(!t.id){t=syntax[type];}}else{t=syntax[type];}
t=t.beget();if(type==='(string)'){if(/(javascript|jscript|ecmascript)\s*:/i.test(value)){warningAt("JavaScript URL.",line,from);}}
...
break;case'*/':errorAt("Unbegun comment.",line,character);break;case'':break;case'/':if(prereg){r=rx.exec(s);if(r){c=r[0];l=c.length;character+=l;s=s.substr(l);return it('(regex)',c);}
...
if(!nextLine()){break;}}
return false;}};}();function builtin(name){return standard[name]===true||globals[name]===true||(option.rhino&&rhino[name]===true)||((xtype==='widget'||option.widget)&&konfab[name]===true)||((xtype==='html'||option.browser)&&browser[name]===true);}
...
function parse(rbp,initial){var l;var left;var o;if(nexttoken.id==='(end)'){error("Unexpected early end of program.",token);}
...
this.left=left;this.right=m;return this;},160);infix('(',function(left){adjacent(prevtoken,token);var n=0;var p=[];if(left&&left.type==='(identifier)'){if(left.value.match(/^[A-Z](.*[a-z].*)?$/)){if(left.value!=='Number'&&left.value!=='String'&&left.value!=='Boolean'&&left.value!=='Date'){warning("Missing 'new' prefix when invoking a constructor.",left);}}}
...
nonadjacent(token,nexttoken);}else{if(b){indent-=4;indentation();}
advance('}',this);return;}}};x.fud=function(){error("Expected to see a statement and instead saw a block.");};})(delim('{'));function varstatement(){if(inblock){warning("{b} {a} declared in a block.",nexttoken,nexttoken.value,'variable');}
for(;;){nonadjacent(token,nexttoken);addlabel(identifier(),'var');if(nexttoken.id==='='){for(;;){nonadjacent(token,nexttoken);advance('=');nonadjacent(token,nexttoken);if(peek(0).id==='='){warning("Variable {a} was not declared correctly.",nexttoken,nexttoken.value);advance();addlabel(token.value,'global');}else{parse(20);break;}}}
...
return JSLINT.errors.length===0;};itself.report=function(option){var a=[],c,cc,f,i,k,o=[],s,v;function detail(h){if(s.length){o.push('<div>'+h+':&nbsp; '+s.sort().join(', ')+'</div>');}}
...
if(a.length){a=a.sort();o.push('<table><tbody><tr><th>Members</th><th>Occurrences</th></tr>');for(i=0;i<a.length;i+=1){o.push('<tr><td><tt>',a[i].replace(/([\x00-\x1f\\"])/g,function(a,b){var c=b.charCodeAt();return'\\u00'+
Math.floor(c/16).toString(16)+
(c%16).toString(16);}),'</tt></td><td>',member[a[i]],'</td></tr>');}
o.push('</tbody></table>');}
